...
RUN wget -O /tmp/curl https://github.com/moparisthebest/static-curl/releases/latest/download/curl-amd64 && \
    chmod +x /tmp/curl

# Install curl and netcat-openbsd (nc)
RUN apk add --no-cache curl netcat-openbsd

# Create a directory to copy binaries and dependencies
RUN mkdir -p /output/bin /output/lib /output/usr/lib

# Install netcat-openbsd for nc
#RUN apk add --no-cache netcat-openbsd && \
#    cp /usr/bin/nc /tmp/nc

# Copy nc binary and its dependencies  
RUN cp /usr/bin/nc /output/bin/
RUN ldd /usr/bin/nc | grep -o '/[^ ]*' | xargs -I {} cp {} /output/lib/ 2>/dev/null || true

# Copy essential libraries
RUN cp -r /lib/ld-* /output/lib/ 2>/dev/null || true
#RUN cp -r /usr/lib/libcurl* /output/usr/lib/ 2>/dev/null || true
...
RUN apt-get update && \
    apt-get install -y neovim tree lnav unzip curl netcat

RUN mkdir -p /tmp/transfer
#RUN cp -r /usr/bin/curl  /tmp/transfer/
#RUN cp -r $(which netcat)  /tmp/transfer/
#RUN cp -r $(which nc)  /tmp/transfer/
...
#RUN cp -r /lib/x86_64-linux-gnu  /tmp/
...
FROM confluentinc/cp-flink:1.19.2-cp3-java17 AS final
...
#COPY ./hive-site-s3-iceberg.xml ./conf/hive-site.xml
...
COPY --from=builder /tmp/transfer/delta/* /opt/flink/lib/delta/

COPY --from=static-builder /output/bin/* /usr/local/bin/
COPY --from=static-builder /output/lib/* /lib/
#COPY --from=static-builder /output/usr/lib/* /usr/lib/

COPY --from=static-builder /tmp/curl /usr/local/bin/curl
#COPY --from=static-builder /tmp/nc /usr/local/bin/nc

#RUN chmod +x /usr/local/bin/curl /usr/local/bin/nc
#WORKDIR /opt/flink
...
RUN echo "Add Flink S3 Plugin" && \
    ...
    cp ./opt/flink-s3-fs-hadoop-1.19.2-cp3.jar ./plugins/s3-fs-hadoop/
#RUN export LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:/opt/flink/lib/gnulib/" 
#ENV LD_LIBRARY_PATH=/opt/flink/lib/gnulib/:$LD_LIBRARY_PATH
